...
def index(request,type='new',page=1):
	collection =[]
	...
	if type=='search':
		...
		if type=='recommand':
			...
			print type
			collection = App_World.objects.all().order_by('?')[:10]
			#category_name = u'전체'
			category_count = Collection.objects.all().count()
		elif type=='all':
...
def events(request,page_category="all",page=1):
	...
	category_dic.append({"category":"all","count":EventManagement.objects.filter(view=1,end_date__gte=date.today()).order_by('-id').count()})
	
	for category_temp in category_filter:
		counting = EventManagement.objects.filter(view=1,category=category_temp['category'],end_date__gte=date.today()).count()
...
def appworld(request,page_category='all',idx=1):
	#page_category 가 detail일때는 상세 페이지로 간다.
	if page_category == 'detail':
		appworld = App_World.objects.get(id=idx)
		relation_category =  App_World.objects.filter(category_id = appworld.category_id).order_by('?')[:4]

		return render_to_response(
			'/appworld_detail.html',
			{
				'request':request,
				'relation_category':relation_category,
				'appworld':appworld
	...
	if page_category == 'all':
		appworld = App_World.objects.all().order_by('-id')
	else:
		category = App_category.objects.get(category_eng = page_category)
		appworld = App_World.objects.filter(category = category.id).order_by('-id')
	tools = page_tools(appworld,idx)
	category_dic=[]
	category_filter = App_World.objects.all().values('category').annotate()
	category_dic.append({"category":"all","category_kor":u"전체","count":appworld.count()})
	for list in category_filter:
		category_temp = App_World.objects.filter(category= list['category'])
		category_dic.append({"category":category_temp[0].category.category_eng,"category_kor":category_temp[0].category.category,"count":category_temp.count()})
